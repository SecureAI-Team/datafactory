...
【重要】判断原则 - 宁可直接搜索，也不要过度询问：
- 只要用户提供了任何具体的领域、行业、产品、场景，就可以直接搜索
- 即使搜索结果可能为空，也应该先尝试搜索，而不是询问更多信息
- 只有当用户完全没有提供任何上下文时（如"找个案例"、"帮我报价"），才需要收集信息

判断规则（is_sufficient=true 的情况）:
- "/案例 有工控安全的案例吗" → 足够！直接搜索"工控安全 案例"
- "有没有工控安全的案例" → 足够！直接搜索"工控安全 案例"
- "汽车行业案例" → 足够！直接搜索"汽车行业 案例"
- "AOI8000的价格" → 足够！直接搜索
- "对比AOI8000和AOI5000" → 足够！直接对比
- "PCB检测方案" → 足够！直接搜索"PCB检测 方案"

判断规则（is_sufficient=false 的情况）:
- "找个案例" → 不够，完全没有指定领域
- "帮我算个报价" → 不够，缺少产品型号、数量
- "有什么方案" → 不够，完全没有指定场景
...
{{
    ...
    "optimized_query": "用于检索的优化查询语句（去掉/命令前缀）",
    "missing_fields": ["缺失的关键信息列表，如果足够则为空数组"],
